...
- **`constructicon-test`** — an isolated instance with its own DB/storage,
  ...
  **Checkout may be ahead of `main`, deliberately**: `constructicon-test`'s
  ...
  2026-09-02, it's on `feat/object-types-stream-url-81` (the #81 stream/url
  object-type migration, following #80 SVG/EPS, #79 PSD, #78 STL, #77 PDF,
  #76 document/audio, and #75 YouTube) — so follow-on migration issue #82
  (wiring up callers + deleting old files) has a live container to test against. Check `git -C
